...
def create_payment_pix():
    ...
    if 'value' not in data:
        return jsonify({"error": "Invalad value"}), 400
    
    expiration_date = datetime.now() + timedelta(minutes=30)

    new_payment = Payment(value=data['value'],
                          experation_date=expiration_date)
    ...
    return jsonify({"message": "The payment has ben created",
                    "payment_id": new_payment.to_dict()})
...
def pix_confirmation():
    ...
    if 'bank_payment_id' not in data and 'value' not in data:
        return jsonify({"error": "Invalad payment data"}), 400
